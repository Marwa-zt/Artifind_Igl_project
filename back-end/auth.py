...
async def login_for_access_token(
    ...
    role = user.role  # Assuming there is a get_role method in your User model
    user_id = user.id
    
    token = create_access_token(user.email, user_id, timedelta(minutes=20))
    if role == "admin":
        return RedirectResponse(url="http://localhost:3000/admin")
    elif role == "moderator":
        return RedirectResponse(url="http://localhost:3000/moderator")
    else:
        return RedirectResponse(url="http://localhost:3000/user")
...
def authenticate_user(form_data: FormData, db: Session):
    ...
    # If it's not a regular user, check if it's an admin
    admin = db.query(Admin).filter(Admin.email == form_data.email).first()

    if admin and bcrypt_context.verify(form_data.password, admin.hashed_password):
        return admin

    # If it's not an admin either, you can check for moderators or other roles...
